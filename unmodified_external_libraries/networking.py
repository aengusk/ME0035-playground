...
#                         self.master._iprint("Message not found in the buffer")
            ...
            def __handle_ack(sender_mac, subtype, stimestamp, rtimestamp, payload_type, payload):
                ...
                if subtype == b'\x10': #Pong
                    ...
                    self.master._iprint(f"Command fail received from {sender_mac} ({self.peer_name(sender_mac)}) for type {payload[0]} with error {payload[1]} and payload {payload[2]}") # modified for syntax error
                    #add to ack buffer
                    ...
                    self.master._iprint(f"Receive confirmation received from {sender_mac} ({self.peer_name(sender_mac)}) for type {payload[0]} with payload {payload[1]}") # modified for syntax error
                    #add to ack buffer
                else:
